...
def sign_de_with_p12(xml_bytes: bytes, p12_path: str, p12_password: str) -> bytes:
    ...
    # FIX XSD v150 (DISABLED): reorder pre-sign se desactiva; usamos reorder post-sign.
    ...
    try:
        ...
        # POST-SIGN REORDER (XSD v150): si existe gCamFuFD dentro de rDE, Signature debe ir ANTES de gCamFuFD.
        # (este endpoint está validando: rDE -> dVerFor, DE, Signature, gCamFuFD)
        try:
            sig_tag = "{http://www.w3.org/2000/09/xmldsig#}Signature"
            def _local(t: str) -> str:
                return t.split('}', 1)[1] if '}' in t else t
            rde_fix = root if _local(getattr(root, 'tag', '')) == 'rDE' else None
            if rde_fix is None:
                rde_fix = next((n for n in root.iter() if _local(getattr(n, 'tag', '')) == 'rDE'), None)
            if rde_fix is not None:
                kids = list(rde_fix)
                sig_el = next((ch for ch in kids if getattr(ch, 'tag', '') == sig_tag), None)
                g_el = next((ch for ch in kids if _local(getattr(ch, 'tag', '')) == 'gCamFuFD'), None)
                if sig_el is not None and g_el is not None:
                    kids2 = list(rde_fix)
                    sig_idx = kids2.index(sig_el)
                    g_idx = kids2.index(g_el)
                    # si Signature quedó DESPUÉS, moverla ANTES
                    if sig_idx > g_idx:
                        rde_fix.remove(sig_el)
                        rde_fix.insert(g_idx, sig_el)
                        try:
                            logger.warning(f"[POST-SIGN REORDER] moví Signature ANTES de gCamFuFD (sig_idx={sig_idx}, g_idx={g_idx})")
                        except Exception:
                            pass
        except Exception as e:
            try:
                logger.warning(f"[POST-SIGN REORDER] no pude reordenar Signature: {e!r}")
            except Exception:
                pass
    ...
    # 3.2) Signature debe ser hijo directo de rDE y en DS_NS_URI (prefijo NO importa)
    # Orden XSD v150 (Remisión, etc.): rDE -> dVerFor, DE, gCamFuFD, Signature
    sig_tag = f"{{{DS_NS_URI}}}Signature"
    children = list(rde_check)

    # ubicar DE como hijo directo
    try:
        de_idx = children.index(de2)
    except ValueError:
        raise XMLSecError("Post-check falló: no se pudo ubicar <DE> como hijo directo de <rDE>")

    # ubicar Signature y gCamFuFD como hijos directos
    sig_idx = next((k for k,ch in enumerate(children) if getattr(ch, "tag", "") == sig_tag), -1)
    g_idx = next((k for k,ch in enumerate(children)
                  if ((getattr(ch, "tag", "").split("}",1)[1] if "}" in getattr(ch, "tag", "") else getattr(ch, "tag", "")) == "gCamFuFD")), -1)

    # Si existe gCamFuFD y Signature quedó antes, mover Signature después de gCamFuFD
    if sig_idx != -1 and g_idx != -1 and sig_idx < g_idx:
        sig_node = children[sig_idx]
        rde_check.remove(sig_node)

        children2 = list(rde_check)
        g_idx2 = next((k for k,ch in enumerate(children2)
                       if ((getattr(ch, "tag", "").split("}",1)[1] if "}" in getattr(ch, "tag", "") else getattr(ch, "tag", "")) == "gCamFuFD")), -1)
        insert_at = g_idx2 + 1 if g_idx2 != -1 else len(children2)
        rde_check.insert(insert_at, sig_node)
    ...
    # Validar existencia de Signature como hijo directo
    if sig_idx == -1:
        sig_any = rde_check.find(f".//{{{DS_NS_URI}}}Signature")
        if sig_any is not None:
            raise XMLSecError("Post-check falló: Signature existe pero NO es hijo directo de <rDE>")
        raise XMLSecError("Post-check falló: no se encontró Signature en DS_NS_URI dentro de <rDE>")

    sig2 = children[sig_idx]

    # Validar orden permitido
    # - sin gCamFuFD: DE -> Signature
    # - con gCamFuFD: DE -> gCamFuFD -> Signature
    if g_idx == -1:
        if sig_idx != de_idx + 1:
            raise XMLSecError("Post-check falló: Signature no está inmediatamente después de <DE> dentro de <rDE> (no hay gCamFuFD)")
    else:
        if not (g_idx == de_idx + 1 and sig_idx == g_idx + 1):
            raise XMLSecError(
                f"Post-check falló: orden inválido dentro de <rDE>. "
                f"Se esperaba DE->gCamFuFD->Signature. Indices: de={de_idx}, gCamFuFD={g_idx}, sig={sig_idx}"
            )
