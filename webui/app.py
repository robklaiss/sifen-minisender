...
def _build_invoice_xml_from_template(
    *,
    ...
    if doc_type == "7":
        gcamcond = gdtip.find("s:gCamCond", ns)
        if gcamcond is None:
            gcamcond = _ensure_child_ns(gdtip, "gCamCond", ns_uri)
            _ensure_child_ns(gcamcond, "iCondOpe", ns_uri).text = "1"
            _ensure_child_ns(gcamcond, "dDCondOpe", ns_uri).text = "Contado"
            gpa = _ensure_child_ns(gcamcond, "gPaConEIni", ns_uri)
            _ensure_child_ns(gpa, "iTiPago", ns_uri).text = "1"
            _ensure_child_ns(gpa, "dDesTiPag", ns_uri).text = "Efectivo"
            # Nota: montos se recalculan en otros pasos; dejamos placeholder mínimo válido
            _ensure_child_ns(gpa, "dMonTiPag", ns_uri).text = "0"
